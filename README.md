...
This is a code to constrain the ultralight dark matter (ULDM) using the SPARC data set. The running modes contain {smart grid, even grid} <img src="https://render.githubusercontent.com/render/math?math=\otimes"> {soliton + NFW, soliton + Burkert, soliton only}. For the first two cases, to speed up the marginalization of nuisance parameters, we recommend using `emcee` to propose a smart grid. 
